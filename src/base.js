...
var new_id = 0;
	...
	,'log':function() {
		...
		var args=[];var m=arguments.length;
		while(--m>=0){args.unshift(arguments[m])}
		document.body.appendChild(Sherd.Base.html2dom('<div class="log">'+String(args)+'</div>'));
	    };
	}
	,'html2dom':function(html_text,doc) {
	    //@html_text MUST have no leading/trailing whitespace, and only be one element
	    doc = (doc)?doc:document;
	    var temp_div = doc.createElement('div');
	    temp_div.innerHTML = html_text;
	    if (temp_div.childNodes.length==1) {return temp_div.firstChild;}
	}
	,'Observer':function() {
		...
		var stor = (_named_listeners[slot_or_pos])?_named_listeners:_listeners;
		if (stor[slot_or_pos]) {
		...
		for (a in _named_listeners) {this.removeListener(a);}
		for (a in _listeners) {this.removeListener(a);}
	    };
	    this.events = {
	...
	,'DomObject':function() {
	    ...
	    this.idPrefix = function() {return 'domObj';}
	    this.id = function() {
		var _dom = this.get();
		if (!_dom.id) {
	...
	,'Storage':function() {
		...
		var localid;
		if (typeof obj_or_id=='string') {localid=obj_ord_id;}
		else if (hasAttr(obj_or_id,'local_id')) {localid = obj_or_id['local_id'];}
		else {localid = String(++localid_counter)}
		return localid;
	    }
...
if (typeof MochiKit != 'undefined') {
	...
	'connect':function(subject,event,self,func){
		...
		|| subject==window || subject==document
	       ) { event = 'on'+event; }
	    var disc = MochiKit.Signal.connect(subject,event,self,func);
	    return {disconnect:function(){MochiKit.Signal.disconnect(disc);}}
	}
	...
	'connect':function(subject,event,self,func){
	    ...
	    return {disconnect:function(){disc.unbind(event);}}
	}
	,'signal':function(subject,event) {
