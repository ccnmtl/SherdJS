...
return dim;},'object2bounds':function(object){var dim=self.openlayers.object_proportioned(object);return new OpenLayers.Bounds(-dim.w,-dim.h,dim.w,dim.h);}};this.Layer=function(){};this.Layer.prototype={all_layers:[],root:{hover:false,click:false,layers:{}},adoptIntoRootContainer:function(new_layer,opts){layerSelf=this;if(this.root.globalMouseListener){this.root.globalMouseListener.destroy();}
this.all_layers.push(new_layer);var listeners=this.root.layers[new_layer.id]={index:this.all_layers.length-1};if(opts.onhover){this.root.hover=true;listeners.onhover=opts.onhover;}
if(opts.onclick){this.root.click=true;listeners.onclick=opts.onclick;}
this.root.globalMouseListener=new OpenLayers.Control.SelectFeature(this.all_layers,{'hover':this.root.hover,overFeature:function(feature){var lay=layerSelf.root.layers[feature.layer.id];if(lay.onhover){lay.onhover(feature.sherd_id,feature.layer.sherd_layername);}},clickFeature:function(feature){var lay=layerSelf.root.layers[feature.layer.id];if(lay.onclick){lay.onclick(feature.sherd_id,feature.layer.sherd_layername);}},outFeature:function(){},highlightOnly:true,renderIntent:"temporary"});self.openlayers.map.addControl(this.root.globalMouseListener);this.root.globalMouseListener.activate();},create:function(name,opts){this.v=new OpenLayers.Layer.Vector(name||"Annotations",{projection:'Flatland:1',rendererOptions:{zIndexing:true,yOrdering:true}});this.name=name;this._anns={};this.v.styleMap=new OpenLayers.StyleMap(self.openlayers.styles);this.v.setZIndex((opts&&opts.zIndex)||200);this.v.sherd_layername=name;self.openlayers.map.addLayer(this.v);this.adoptIntoRootContainer(this.v,opts);return this;},destroy:function(){this.all_layers.splice(this.root.layers[this.v.id].index,1);delete this.root.layers[this.v.id];this.v.destroy();for(ann_id in this._anns)
...
if(opts.color){if(!(opts.color in this.v.styleMap.styles)){this.v.styleMap.styles[opts.color]=new OpenLayers.Style({fillOpacity:0,strokeWidth:2,strokeColor:opts.color,pointerEvents:(opts.pointerEvents),graphicZIndex:(opts.zIndex||300-parseInt(feature_fg.geometry.getBounds().top))});}
...
var projection='Flatland:1';self.openlayers.vectors=new OpenLayers.Layer.Vector("Vector Layer",{projection:projection,rendererOptions:{zIndexing:true,yOrdering:false}});self.openlayers.styles={'black':new OpenLayers.Style({fillOpacity:0,strokeWidth:4,strokeColor:'#000000',pointerEvents:'none',graphicZIndex:0}),'defaulta':new OpenLayers.Style({fillOpacity:0,strokeWidth:2,graphicZIndex:0}),'white':new OpenLayers.Style({fillOpacity:0,strokeWidth:4,strokeColor:'#ffffff',pointerEvents:'none',labelSelect:false,graphicZIndex:0}),'defaultx':new OpenLayers.Style({fillOpacity:0,strokeWidth:2,pointerEvents:'none',labelSelect:false,graphicZIndex:0})};self.openlayers.vectors.styleMap=new OpenLayers.StyleMap(self.openlayers.styles);self.openlayers.map.addControl(new OpenLayers.Control.MousePosition());self.openlayers.map.addLayers([self.openlayers.graphic,self.openlayers.vectors]);self.openlayers.vectors.setZIndex(900);self.openlayers.GeoJSON=new OpenLayers.Format.GeoJSON({'internalProjection':self.openlayers.map.baseLayer.projection,'externalProjection':new OpenLayers.Projection(projection)});presentation.initialize(create_obj.object,self);}};this.microformat.components=function(html_dom,create_obj){return{'top':html_dom};};this.queryformat={find:function(str){return[];},read:function(found_obj){}};};}
