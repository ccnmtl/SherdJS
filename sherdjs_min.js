...
return dim;},'object2bounds':function(object){var dim=self.openlayers.object_proportioned(object);return new OpenLayers.Bounds(-dim.w,-dim.h,dim.w,dim.h);}};this.Layer=function(){};this.Layer.prototype={_anns:{},create:function(name,opts){this.v=new OpenLayers.Layer.Vector(name||"Annotations",{projection:'Flatland:1'});this.v.styleMap=new OpenLayers.StyleMap(self.openlayers.styles);self.openlayers.map.addLayers([this.v]);this.v.setZIndex(200);if(opts){if(opts.onhover){this.onhover=new OpenLayers.Control.SelectFeature(this.v,{'hover':true,'overFeature':opts.onhover,outFeature:function(){},highlightOnly:true,renderIntent:"temporary"});self.openlayers.map.addControl(this.onhover);this.onhover.activate();}
