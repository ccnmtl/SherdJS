...
var listeners=this.root.layers[new_layer.id]={};this.all_layers.push(new_layer);this.all_layers.sort(function(a,b){return a.sherd_layerapi.zIndex-b.sherd_layerapi.zIndex;})
for(var i=0,l=this.all_layers.length;i<l;i++){var layer=this.all_layers[i]
this.root.layers[layer.id].index=i;layer.map.setLayerIndex(layer,i+1);}
...
if(opts.color){if(!(opts.color in this.v.styleMap.styles)){this.v.styleMap.styles[opts.color]=new OpenLayers.Style({fillOpacity:0,strokeWidth:1,strokeColor:opts.color,pointerEvents:(opts.pointerEvents),graphicZIndex:(opts.zIndex||300-parseInt(feature_fg.geometry.getBounds().top))});}
feature_fg.renderIntent=opts.color;}
if(opts.bgcolor){feature_bg.renderIntent=opts.bgcolor;}}
this.v.addFeatures(features);},remove:function(ann_id){if(this.v&&ann_id in this._anns){this.v.removeFeatures(this._anns[ann_id].f);var o=this._anns[ann_id].opts;delete this._anns[ann_id];return o;}},removeAll:function(){if(!this.v)return;this.v.removeAllFeatures();for(ann_id in this._anns)
delete this._anns[ann_id];},show:function(){this.v.setVisibility(true);},hide:function(){this.v.setVisibility(false);},getLayer:function(){return this.v;}}
...
var mode='create';this.setState=function(obj){if(typeof obj=='object'){self.openlayers.editingtoolbar.deactivate();self.components.center.style.display='inline';self.components.redo.style.display='inline';self.components.instructions.style.display='none';mode='browse';}};this.initialize=function(create_obj){self.openlayers.editingtoolbar=new self.openlayers.CustomEditingToolbar(self.targetview.openlayers.vectorLayer.getLayer());self.targetview.openlayers.map.addControl(self.openlayers.editingtoolbar);OpenLayers.Control.prototype.activate.call(self.openlayers.editingtoolbar.sherd.navigation);self.openlayers.editingtoolbar.featureAdded=function(feature){var current_state=self.targetview.getState();var geojson=self.targetview.openlayers.feature2json(feature);for(a in geojson){current_state[a]=geojson[a]};geojson.preserveCurrentFocus=true;self.targetview.setState(geojson);self.storage.update(current_state);}
self.events.connect(self.components.center,'click',function(evt){self.targetview.setState({feature:self.targetview.currentfeature});});self.events.connect(self.components.redo,'click',function(evt){if(mode!='create'){mode='create';self.openlayers.editingtoolbar.activate();self.openlayers.editingtoolbar.sherd.squareHandler.activate();self.targetview.currentfeature=false;self.targetview.openlayers.vectorLayer.removeAll();self.components.instructions.style.display='inline';self.components.center.style.display='none';self.components.redo.style.display='none';}});}
