...
var listeners=this.root.layers[new_layer.v.id]={me:new_layer};this.all_layers.push(new_layer.v);this.all_layers.sort(function(a,b){return a.sherd_layerapi.zIndex-b.sherd_layerapi.zIndex;})
...
if(opts.color){if(opts.color in this.v.styleMap.styles){feature_fg.renderIntent=opts.color;}else{var feature_style=feature_fg.id+':'+opts.color
this.v.styleMap.styles[feature_style]=new OpenLayers.Style({fillOpacity:0,strokeWidth:1,strokeColor:opts.color,pointerEvents:(opts.pointerEvents),graphicZIndex:(opts.zIndex||300-parseInt(feature_fg.geometry.getBounds().top))});feature_fg.renderIntent=feature_style;}}
...
return{object:obj,htmlID:wrapperID,text:'<div id="'+wrapperID+'" class="sherd-openlayers-map"></div>'};};this.microformat.update=function(obj,html_dom){};this.deinitialize=function(){if(this.openlayers.map){var lays=this.Layer.prototype.root.layers;for(a in lays){if(lays[a].name!='annotating')
lays[a].me.destroy();}
this.openlayers.map.destroy();}}
this.initialize=function(create_obj){if(create_obj){var top=document.getElementById(create_obj.htmlID);var presentation;switch(typeof create_obj.object.presentation){case'string':presentation=self.presentations[create_obj.object.presentation];break;case'object':presentation=create_obj.object.presentation;break;case'undefined':presentation=self.presentations['default'];break;}
