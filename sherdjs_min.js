...
return dim;},'object2bounds':function(object){var dim=self.openlayers.object_proportioned(object);return new OpenLayers.Bounds(-dim.w,-dim.h,dim.w,dim.h);}};this.Layer=function(){};this.Layer.prototype={_anns:{},create:function(name,opts){this.v=new OpenLayers.Layer.Vector(name||"Annotations",{projection:'Flatland:1'});this.v.styleMap=new OpenLayers.StyleMap(self.openlayers.styles);self.openlayers.map.addLayers([this.v]);this.v.setZIndex((opts&&opts.zIndex)||200);if(opts){if(opts.onhover){this.onhover=new OpenLayers.Control.SelectFeature(this.v,{'hover':true,'overFeature':opts.onhover,outFeature:function(){},highlightOnly:true,renderIntent:"temporary"});self.openlayers.map.addControl(this.onhover);this.onhover.activate();}
...
if(this.onhover){self.openlayers.map.removeControl(this.onhover);}},add:function(ann,opts){var feature_bg=self.openlayers.GeoJSON.parseFeature(ann);var feature_fg=feature_bg.clone();var features=[feature_bg,feature_fg];feature_bg.renderIntent='black';feature_fg.renderIntent='defaulta';if(opts){if(opts.id)
...
var projection='Flatland:1';self.openlayers.vectors=new OpenLayers.Layer.Vector("Vector Layer",{projection:projection});self.openlayers.styles={'black':new OpenLayers.Style({fillOpacity:0,strokeWidth:4,strokeColor:'#000000'}),'defaulta':new OpenLayers.Style({fillOpacity:0,strokeWidth:2}),'white':new OpenLayers.Style({fillOpacity:0,strokeWidth:4,strokeColor:'#ffffff',pointerEvents:'none',labelSelect:false}),'defaultx':new OpenLayers.Style({fillOpacity:0,strokeWidth:2,pointerEvents:'none',labelSelect:false})};self.openlayers.vectors.styleMap=new OpenLayers.StyleMap(self.openlayers.styles);self.openlayers.map.addControl(new OpenLayers.Control.MousePosition());self.openlayers.map.addLayers([self.openlayers.graphic,self.openlayers.vectors]);self.openlayers.vectors.setZIndex(900);self.multiple=new this.Layer().create()
