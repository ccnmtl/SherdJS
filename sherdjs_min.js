...
cur.clipstrip.html.push(target,{asset:{}});self.clipstrip=cur.clipstrip;}}else{if(window.console){console.log(options);}
throw Error("Your asset does not have a (supported) type marked.");}}};this.setState=function(){if(current_type){var cur=self.settings[current_type];cur.view.setState.apply(cur.view,arguments);if(self.clipstrip){self.clipstrip.setState.apply(self.clipstrip,arguments);}}};this.getState=function(){if(current_type){return self.settings[current_type].view.getState.apply(self.settings[current_type].view,arguments);}};this.queryformat={find:function(str){if(self.settings[current_type].view.queryformat&&self.settings[current_type].view.queryformat.find){return self.settings[current_type].view.queryformat.find(str);}else{return[];}}};};}
...
return dim;},'object2bounds':function(object){var dim=self.openlayers.object_proportioned(object);return new OpenLayers.Bounds(-dim.w,-dim.h,dim.w,dim.h);}};this.Layer=function(){};this.Layer.prototype={_anns:{},create:function(name){this.v=new OpenLayers.Layer.Vector(name||"Annotations",{projection:'Flatland:1'});this.v.styleMap=new OpenLayers.StyleMap(self.openlayers.styles);self.openlayers.map.addLayers([this.v]);return this;},destroy:function(){this.v.destroy();for(ann_id in this._anns)
delete this._anns[ann_id];},add:function(ann,opts){var feature_bg=self.openlayers.GeoJSON.parseFeature(ann);var feature_fg=feature_bg.clone();var features=[feature_bg,feature_fg];feature_bg.renderIntent='black';feature_fg.renderIntent='defaultx';if(opts){if(opts.id)
this._anns[opts.id]=features;if(opts.color){if(!opts.color in this.v.styleMap.styles){this.v.styleMap.styles[opts.color]=new
OpenLayers.Style({fillOpacity:0,strokeWidth:2,strokeColor:opts.color});}
feature_fg.renderIntent=opts.color;}}
this.v.addFeatures(features);},remove:function(ann_id){if(ann_id in this._anns){this.v.removeFeatures(this._anns[ann_id]);delete this._anns[ann_id];}},removeAll:function(){this.v.removeAllFeatures();for(ann_id in this._anns)
delete this._anns[ann_id];},show:function(){this.v.display(true);},hide:function(){this.v.display(false);}}
